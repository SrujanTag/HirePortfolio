  ...
  if (HIRE_KEYWORDS.some(k => q.includes(k))) {
    ...
    if (matched.length > 0) {
      ...
      return {
        ...
        actions: [{
          label: `💼 Hire ${person.name.split(' ')[0]}`,
          action: () => onNavigate('hire', person)
        }, {
          label: '📂 View Profile',
          action: () => onNavigate('profile', person)
        }]
      };
    }
    return {
  ...
  if (skillMatch.length > 0) {
    ...
    return {
      ...
      actions: [{
        ...
        label: `💼 Hire ${skillMatch[0].name.split(' ')[0]}`,
        action: () => onNavigate('hire', skillMatch[0])
      }] : [])]
    };
  }
...
function TypingDots() {
  return <div className="flex items-center gap-1 px-4 py-3">
      {[0, 1, 2].map(i => <div key={i} className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{
...
function Bubble({
  ...
  return <div className={`flex ${isBot ? 'justify-start' : 'justify-end'} mb-3`}>
      {isBot && <div className="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold shrink-0 mr-2 mt-0.5">
          🤖
        </div>}
      <div className={`max-w-[80%] ${isBot ? 'bg-[#1C1F26] border border-[#2F333A] text-gray-200 rounded-2xl rounded-tl-sm' : 'bg-blue-600 text-white rounded-2xl rounded-tr-sm'} px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap`}>
        {msg.text}
        {msg.actions?.length > 0 && <div className="flex flex-col gap-1.5 mt-3">
            {msg.actions.map((a, i) => <button key={i} onClick={a.action} className="w-full text-left px-3 py-1.5 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 text-blue-300 text-xs font-medium transition-colors">
                {a.label}
              </button>)}
          </div>}
      </div>
...
const QUICK_REPLIES = ['👥 Show team', '💼 Hire talent', '⚡ React devs', '✉️ Contact'];
const ChatBot = ({
  users = [],
  onNavigate
}) => {
  ...
  const sendMessage = text => {
    ...
    setTimeout(() => {
      ...
      const reply = botReply(trimmed, users, (page, person) => {
        setOpen(false);
        if (page === 'hire') onNavigate?.('hire', person);else if (page === 'profile') onNavigate?.('profile', person);else onNavigate?.(page);
      });
      pushMessage({
        sender: 'bot',
        ...reply
  ...
  return <>
      <div className={`fixed bottom-24 right-4 z-[9990] w-80 sm:w-96 flex flex-col
          bg-[#0F1117] border border-[#1F232C] rounded-2xl shadow-2xl
          transition-all duration-300 origin-bottom-right
          ${open ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-90 pointer-events-none'}`} style={{
      maxHeight: '520px'
    }}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#1F232C] bg-gradient-to-r from-blue-900/30 to-purple-900/20 rounded-t-2xl">
          ...
          <button onClick={() => setOpen(false)} className="text-gray-500 hover:text-white text-xl leading-none transition-colors">
            ×
          </button>
        </div>
        ...
        minHeight: 0
          ...
          {typing && <div className="flex justify-start mb-3">
        ...
        <div className="flex gap-1.5 px-4 pb-2 flex-wrap">
          {QUICK_REPLIES.map(q => <button key={q} onClick={() => sendMessage(q.replace(/^[^\s]+ /, ''))} className="text-[10px] px-2.5 py-1 rounded-full bg-[#1C1F26] border border-[#2F333A] text-gray-400 hover:text-white hover:border-gray-500 transition-all">
              {q}
            </button>)}
        </div>
        <div className="flex items-center gap-2 px-4 py-3 border-t border-[#1F232C]">
          <input ref={inputRef} type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="Ask me anything..." className="flex-1 bg-[#161922] border border-[#2F333A] rounded-xl px-3 py-2 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-blue-500/50 transition-colors" />
          <button onClick={() => sendMessage()} disabled={!input.trim()} className="w-9 h-9 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:opacity-50 flex items-center justify-center text-white transition-all shrink-0">
      ...
      <button onClick={() => setOpen(o => !o)} className={`fixed bottom-4 right-4 z-[9991] w-14 h-14 rounded-full shadow-2xl
          flex items-center justify-center text-2xl
          bg-gradient-to-br from-blue-600 to-purple-600
          hover:from-blue-500 hover:to-purple-500
          transition-all duration-300
          ${open ? 'rotate-0 scale-95' : 'rotate-0 scale-100 hover:scale-110'}`} title="Chat with LeapBot">
        {open ? <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />
          </svg> : <span>💬</span>}
        {!open && unread > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center animate-pulse">
            {unread}
          </span>}
      </button>
    </>;
